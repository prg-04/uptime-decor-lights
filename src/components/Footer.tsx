  ...
  return (
    <footer className="bg-black/90 text-white py-12 px-6">
                  ...
                  <li key={index}>
                    {service.href ? (
                      ...
                      >
                        ...
                        <div>
                          {service.title && <p className="font-medium">{service.title}</p>}
                          {service.text && <p>{service.text}</p>}
                          {service.subText && (
                            <p className="text-xs text-white/60">{service.subText}</p>
                          )}
                        </div>
                      </Link>
                    ) : (
      ...
      <div className="text-center text-xs text-white/50 mt-12">
        &copy; {new Date().getFullYear()} Uptime Decor Lights. All rights reserved.
      </div>
    </footer>
  )
}
